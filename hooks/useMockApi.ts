...
const sendAssignmentEmail = async (user: User, entry: LogEntry) => {
  if (!user.email) {
    console.warn(
      `El usuario ${user.fullName} no tiene un email configurado para recibir notificaciones.`
    );
    return;
  }
  console.log(
    `SIMULACIÓN: Enviando email de asignación a ${user.email} para el folio #${entry.folioNumber}`
  );
...
const sendCommitmentReminderEmail = async (
  ...
  if (!user.email) {
    console.warn(
      `El usuario ${user.fullName} no tiene un email configurado para recibir recordatorios.`
    );
    return;
  }
  console.log(
    `SIMULACIÓN: Enviando email de recordatorio de compromiso a ${user.email}`
  );
  ...
  console.log(
    `   - Fecha Límite: ${new Date(commitment.dueDate).toLocaleDateString(
      "es-CO"
    )}`
  );
  // En una aplicación real, aquí iría la llamada fetch a un endpoint de backend.
...
export const useMockApi = () => {
  ...
  const [contractModifications, setContractModifications] = useState<
    ContractModification[]
  >([]);
  ...
  const addEntry = async (
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const newFolio =
          logEntries.length > 0
            ? Math.max(...logEntries.map((e) => e.folioNumber)) + 1
            : 1001;
      ...
      setTimeout(() => {
        ...
        setLogEntries((prev) => {
          ...
          const fieldDisplayNames: {
            [key in keyof Omit<LogEntry, "assignees">]?: string;
          } = {
            title: "Título",
          ...
          (
            Object.keys(fieldDisplayNames) as Array<
              keyof typeof fieldDisplayNames
            >
          ).forEach((field) => {
          ...
          const originalAttachmentIds = new Set(
            originalAttachments.map((a) => a.id)
          );
          const updatedAttachmentIds = new Set(
            updatedAttachments.map((a) => a.id)
          );
  ...
  const updateActa = async (updatedActa: Acta): Promise<void> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        setActas((prev) =>
          prev.map((acta) => (acta.id === updatedActa.id ? updatedActa : acta))
        );
        resolve();
      }, API_DELAY);
    });
  ...
  const addCostActa = async (
    newCostActaData: Omit<CostActa, "id" | "observations" | "attachments">,
    files: File[]
  ): Promise<void> => {
    return new Promise((resolve) => {
      setTimeout(() => {
  ...
  const updateCostActa = async (updatedCostActa: CostActa): Promise<void> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (
          finalActa.status === CostActaStatus.APPROVED &&
          !finalActa.approvalDate
        ) {
        ...
        setCostActas((prev) =>
          prev.map((acta) => (acta.id === finalActa.id ? finalActa : acta))
        );
        resolve();
      }, API_DELAY);
    });
          ...
          prev.map((comm) => {
            if (comm.id === commId) {
              ...
              return {
                ...comm,
                status: newStatus,
                statusHistory: [...comm.statusHistory, newHistoryEntry],
              };
            }
            return comm;
          })
  ...
  const addWorkActa = async (
    newActaData: Omit<WorkActa, "id">
  ): Promise<void> => {
    return new Promise((resolve) => {
      setTimeout(() => {
  ...
  const addContractModification = async (
  ...
  const addControlPoint = async (
    newPointData: Omit<ControlPoint, "id" | "photos">
  ): Promise<void> => {
    return new Promise((resolve) => {
      setTimeout(() => {
  ...
  const addPhotoToControlPoint = async (
    pointId: string,
    photoData: Omit<PhotoEntry, "id" | "author" | "date">,
    file: File,
    author: User
  ): Promise<void> => {
    return new Promise((resolve) => {
        ...
        setControlPoints((prev) =>
          prev.map((point) =>
            point.id === pointId
              ? {
                  ...point,
                  photos: [...point.photos, newPhoto].sort(
                    (a, b) =>
  ...
  const addReport = async (
    newReportData: Omit<Report, "id" | "author" | "status" | "attachments">,
    files: File[],
    author: User
  ): Promise<void> => {
    return new Promise((resolve) => {
  ...
  const updateReport = async (updatedReport: Report): Promise<void> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        setReports((prev) =>
          prev.map((rep) => (rep.id === updatedReport.id ? updatedReport : rep))
        );
        resolve();
      }, API_DELAY);
    });
  ...
  const addWeeklyReport = async (
    reportData: Omit<WeeklyReport, "id">
  ): Promise<void> => {
    return new Promise((resolve) => {
      setTimeout(() => {
  ...
  const addSignature = async (
    documentId: string,
    documentType: "logEntry" | "acta" | "report",
    signer: User
  ): Promise<LogEntry | Acta | Report | undefined> => {
    return new Promise((resolve) => {
      setTimeout(() => {
            ...
            setLogEntries((prev) => {
              const newEntries = prev.map((entry) => {
                if (entry.id === documentId) {
                  ...
                  if (
                    entry.requiredSignatories.length > 0 &&
                    updatedSignatures.length >= entry.requiredSignatories.length
                  ) {
            ...
            setActas((prev) => {
              const newActas = prev.map((acta) => {
                if (acta.id === documentId) {
                  ...
                  if (
                    acta.status === ActaStatus.FOR_SIGNATURES &&
                    acta.requiredSignatories.length > 0 &&
                    updatedSignatures.length >= acta.requiredSignatories.length
                  ) {
              ...
              const newReports = prev.map((report) => {
                if (report.id === documentId) {
                  ...
                  if (
                    ...
                    report.requiredSignatories.length > 0 &&
                    updatedSignatures.length >=
                      report.requiredSignatories.length
                  ) {
  ...
  const addCommentToDrawing = async (
    drawingId: string,
    commentText: string,
    author: User
