...
import { MOCK_TOTAL_CONTRACT_VALUE } from '../services/mockData';
...
const CostDashboard: React.FC<CostDashboardProps> = ({ project, api }) => {
  ...
  const handleSaveActa = async (newActaData: Omit<CostActa, 'id' | 'observations' | 'attachments'>, files: File[]) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Seguimiento de Actas de Costos</h2>
          <p className="text-sm text-gray-500">Contrato de Interventoría: {project.contractId}</p>
        </div>
      ...
      {isLoading && <div className="text-center p-8">Cargando actas de costos...</div>}
                ...
                <tbody>
                    {costActas.map(acta => (
                        <tr key={acta.id} className="bg-white border-b hover:bg-gray-50 cursor-pointer" onClick={() => handleOpenDetail(acta)}>
                            ...
                            <td className="px-6 py-4">{((acta.billedAmount / acta.totalContractValue) * 100).toFixed(2)}%</td>
