  ...
  const handleCommitmentToggle = (commitmentId: string) => {
    setEditedActa(prevActa => ({
        ...
        commitments: prevActa.commitments.map(c => 
            c.id === commitmentId
            ? { ...c, status: c.status === CommitmentStatus.PENDING ? CommitmentStatus.COMPLETED : CommitmentStatus.PENDING }
            : c
        )
    }));
  };
        ...
        <div className="space-y-6">
            <div className="pb-4 border-b">
                ...
                <div className="flex items-center gap-4 mt-2">
                    <p className="text-sm text-gray-500">Fecha: {new Date(date).toLocaleDateString('es-CO', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
            ...
            <div>
                <h4 className="text-md font-semibold text-gray-800">Resumen de la Reunión</h4>
                            ...
                            <li key={commitment.id} className="p-4 flex flex-col sm:flex-row justify-between sm:items-start gap-3">
                                <div className="flex-1 flex items-start">
                                    <input
                                        ...
                                        checked={commitment.status === CommitmentStatus.COMPLETED}
                                        onChange={() => handleCommitmentToggle(commitment.id)}
                                    />
                                    <label htmlFor={`commitment-${commitment.id}`} className="ml-3 flex-1">
                                        <p className={`text-sm text-gray-800 ${commitment.status === CommitmentStatus.COMPLETED ? 'line-through text-gray-500' : ''}`}>
                                ...
                                <div className="flex-shrink-0 flex items-center gap-4 pl-8 sm:pl-0">
                                     {commitment.status === CommitmentStatus.PENDING && (
                                        <button onClick={() => handleSendReminderClick(commitment)} title="Enviar recordatorio por email" className="text-gray-400 hover:text-brand-primary">
                                            <EmailIcon className="h-5 w-5"/>
                                        </button>
                                     )}
                                     <span className={`text-sm font-semibold ${commitment.status === CommitmentStatus.COMPLETED ? 'text-green-600' : 'text-yellow-600'}`}>{commitment.status}</span>
                                     <div className={`flex items-center text-xs font-medium ${getDueDateColor(commitment.dueDate, commitment.status)}`}>
